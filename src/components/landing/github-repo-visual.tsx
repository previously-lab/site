...
interface GitHubRepoVisualProps {
  repoName: string;
...
export function GitHubRepoVisual({ repoName, repoVisibility, lockText }: GitHubRepoVisualProps) {
  const ref = useRef<HTMLDivElement>(null);
  const isInView = useInView(ref, { once: true, margin: "-100px" });

  return (
    <div ref={ref} className="w-full max-w-xl" role="img"
      aria-label="Your memory lives as Markdown files in your own GitHub repository">
      <motion.div className="flex items-center gap-2 rounded-t-lg border border-border bg-muted/30 px-4 py-2.5"
        initial={{ opacity: 0, y: -4 }} animate={isInView ? { opacity: 1, y: 0 } : { opacity: 0, y: -4 }}
        ...
        <span className="font-mono text-xs font-medium text-foreground/80 sm:text-sm">{repoName}</span>
        <span className="ml-auto font-mono text-[0.6rem] text-muted-foreground/50">{repoVisibility}</span>
      </motion.div>

      <motion.div className="rounded-b-lg border border-t-0 border-border bg-card px-4 py-3 font-mono text-xs sm:text-sm"
        initial={{ opacity: 0 }} animate={isInView ? { opacity: 1 } : { opacity: 0 }}
        transition={{ duration: 0.5, delay: 0.2 }}>
        {FILE_TREE.map((item, i) => (
          <motion.div key={i} className="flex items-center gap-1.5 leading-relaxed"
            style={{ paddingLeft: `${item.indent * 1}rem` }}
            initial={{ opacity: 0, x: -4 }} animate={isInView ? { opacity: 1, x: 0 } : { opacity: 0, x: -4 }}
      ...
      <motion.div className="mt-4 flex items-center justify-center gap-2"
        ...
        <span className="font-mono text-[0.65rem] text-muted-foreground/60">{lockText}</span>
      </motion.div>
    </div>
  );
}

...
export function OpenSourceVisual({
  badgeMIT, badgeSelfHost, badgeNoTelemetry, badgeCommunity,
  githubStars, githubMIT,
}: OpenSourceVisualProps) {
  const ref = useRef<HTMLDivElement>(null);
  const isInView = useInView(ref, { once: true, margin: "-100px" });

  const badges = [badgeMIT, badgeSelfHost, badgeNoTelemetry, badgeCommunity];

  return (
    <div ref={ref} className="flex flex-wrap items-center justify-center gap-3"
      role="img" aria-label={badges.join(", ")}>
      {badges.map((label, i) => (
        <motion.div key={i}
          ...
          <svg viewBox="0 0 16 16" className="size-3.5 shrink-0 text-muted-foreground/50" fill="none" stroke="currentColor" strokeWidth="2" aria-hidden="true">
            <motion.path d="M3 8l3.5 3.5L13 5"
              initial={{ pathLength: 0 }} animate={isInView ? { pathLength: 1 } : { pathLength: 0 }}
              transition={{ duration: 0.3, delay: 0.5 + i * 0.12 }} strokeDasharray="1" strokeDashoffset="0" />
          </svg>
          <span className="text-xs font-medium text-foreground/70 sm:text-sm">{label}</span>
        </motion.div>
      ))}

      <motion.div className="mt-6 flex w-full items-center justify-center gap-6"
        ...
        <div className="flex items-center gap-1.5">
          ...
          <span className="font-mono text-xs text-muted-foreground/70">{githubStars}</span>
        </div>
        ...
        <div className="flex items-center gap-1.5">
          ...
          <span className="font-mono text-xs text-muted-foreground/70">{githubMIT}</span>
        </div>
      </motion.div>
    </div>
  );

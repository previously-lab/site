...
interface BeliefNode {
  ...
  cy: number;
  label: string;
  category: "identity" | "pattern" | "strategy" | "context";
  delay: number;
  /** If true, this node fades out after appearing (represents "expire") */
  fadesOut?: boolean;
}
...
const CONNECTIONS: Connection[] = [
  ...
  { from: 2, to: 4, delay: 1.3 },
  { from: 3, to: 5, delay: 1.5 },
  { from: 4, to: 6, delay: 1.7 },
  { from: 5, to: 7, delay: 1.9 },
  { from: 6, to: 8, delay: 2.1 },
  { from: 1, to: 3, delay: 1.2 },
  { from: 2, to: 6, delay: 1.6 },
];
...
/**
 * Screen 3 visual: constellation of belief-nodes representing the agent's
 * evolving self-model. Nodes appear with staggered delays, connections
 * form between related beliefs, and one node fades out (representing the
 * "expire" cognitive operation). Abstract, poetic, non-literal.
 */
  ...
  return (
    ...
    >
      <svg
        ...
        aria-hidden="true"
      >
        {/* Connections — thin lines between related nodes */}
        {CONNECTIONS.map((conn, i) => {
          ...
          return (
            <motion.line
              ...
              transition={{ duration: 0.5, delay: conn.delay }}
              strokeDasharray="1"
              strokeDashoffset="0"
            />
          );
        ...
        {NODES.map((node) => (
          <motion.g
            ...
            animate={
              isInView
                ? node.fadesOut
                  ? { opacity: [0, 1, 1, 0], scale: [0, 1, 1, 0.5] }
                  : { opacity: 1, scale: 1 }
                : { opacity: 0, scale: 0 }
            }
            transition={
          ...
          >
            {/* Outer ring — subtle pulse for identity nodes */}
            <motion.circle
              cx={node.cx}
              cy={node.cy}
              r="14"
              strokeWidth="1"
              className={CATEGORY_RINGS[node.category]}
              fill="none"
              initial={{ scale: 1 }}
              animate={isInView && !node.fadesOut ? { scale: [1, 1.3, 1] } : { scale: 1 }}
              transition={
                isInView && !node.fadesOut
                  ? { duration: 3, delay: node.delay + 1, repeat: Infinity, repeatDelay: 2 }
                  : {}
              }
            />
            {/* Core dot */}
            <circle
              ...
              r="4"
              strokeWidth="1.5"
              className={CATEGORY_COLORS[node.category]}
            />
            {/* Label */}
            <text
              x={node.cx}
        ...
        {/* Center label — appears after all nodes */}
        <motion.text
          x="320"
          y="145"
          textAnchor="middle"
          className="fill-muted-foreground/50 text-[9px] italic"
          initial={{ opacity: 0 }}
          animate={isInView ? { opacity: 1 } : { opacity: 0 }}
          transition={{ duration: 0.5, delay: 2.2 }}
        >
          Evolving understanding — observe, reinforce, correct, expire
        </motion.text>
      </svg>
    </div>
  );

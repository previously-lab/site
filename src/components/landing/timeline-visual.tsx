...
// Scattered positions — naturally centered, no label to avoid
const SCATTERED = [
  { cx: 140, cy: 40 },
  { cx: 420, cy: 30 },
  { cx: 670, cy: 44 },
...
const STRANDS = [
  { ids: [0, 2, 5], color: "#0066FF", label: "Work", labelX: 218 },
  { ids: [1, 3, 4], color: "oklch(0.7 0.12 85)", label: "Travel", labelX: 336 },
] as const;

/* ── Card sub-component ───────────────────────────────── */

function Card({
  cx, cy, label, muted, isHovered,
}: {
  ...
  return (
    <g>
      ...
      {isHovered && (
        <rect
          x={cx - halfW - 3} y={cy - halfH - 3}
          width={CARD_W + 6} height={CARD_H + 6} rx={7}
          className="fill-[#0066FF]/8"
        />
      )}
      <rect
...
export function TimelineVisual({
  ...
  return (
    <div ref={ref} className="w-full" role="img" aria-label={`${beforeLabel}. ${afterLabel}.`}>
      {/* ═══════════════════════════════════════════════════
          TOP — "Before": scattered, isolated chat threads
         ═══════════════════════════════════════════════════ */}
      <div className="mb-6">
        {/* Label — real HTML, never overlaps SVG content */}
        <motion.p
          className="mb-6 text-center text-[11px] font-medium text-muted-foreground/60"
          initial={{ opacity: 0 }}
        ...
        <svg viewBox="0 0 800 170" fill="none" xmlns="http://www.w3.org/2000/svg" className="w-full" aria-hidden="true">
          {beforeCardLabels.slice(0, 6).map((label, i) => {
            ...
            return (
              <motion.g
                ...
                animate={isInView
                  ? { opacity: 1, y: 0, scale: isHovered ? 1.06 : 1 }
                  : { opacity: 0, y: -6 }}
      ...
      {/* Divider — HTML, not SVG */}
      <motion.hr
        className="my-10 border-border/20"
      ...
      {/* ═══════════════════════════════════════════════════
          BOTTOM — "After": continuous timeline with strands
         ═══════════════════════════════════════════════════ */}
      <div className="mt-6">
        <motion.p
          className="mb-6 text-center text-[11px] font-semibold text-foreground/70"
          initial={{ opacity: 0 }}
          animate={isInView ? { opacity: 1 } : { opacity: 0 }}
        ...
        <svg viewBox="0 0 800 200" fill="none" xmlns="http://www.w3.org/2000/svg" className="w-full" aria-hidden="true">
          {/* Timeline line */}
          <motion.line
            x1={TIMELINE_X[0]} y1={DOT_Y} x2={NOW_X} y2={DOT_Y}
          ...
          {STRANDS.map((strand, si) => {
            ...
            const d = points
              .map((x, pi) => {
                ...
                const midX = (points[pi - 1] + x) / 2;
                return `Q ${midX} ${arcY - 22} ${x} ${arcY}`;
              })
              .join(" ");
            return (
              <motion.g
          ...
          {afterCardLabels.slice(0, 6).map((label, i) => {
            ...
            return (
              ...
              >
                ...
                />
                <text x={cx} y={DOT_Y + 16} textAnchor="middle" className="fill-muted-foreground/50 text-[7px]">
                  {DATES[i]}
                </text>
              </motion.g>
            );
          ...
          >
            ...
            <text x={NOW_X} y={DOT_Y + 16} textAnchor="middle" className="fill-foreground text-[8px] font-bold">NOW</text>
          </motion.g>

          <motion.text
            x={TIMELINE_X[0] - 12} y={DOT_Y + 5} textAnchor="end"

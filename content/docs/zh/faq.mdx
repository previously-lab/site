---
title: 常见问题
description: 关于 Previously 如何工作、运行成本以及边界在哪里的常见问题。
order: 12
---
...
你的记忆就是一个 git 仓库里 Markdown 文件的目录树。仅此而已。没有数据库，没有向量存储，没有专有格式。
</Callout>
...
其机制位于 `src/lib/context/assembler.ts`。每次请求都会获得一次全新的上下文组装，默认预算为 8,000 token。当预算超出时（保留 500 token 空间），block 停止加载。这意味着时间线可以跨越数月的对话不断增长，但加载到任何单次请求中的内容始终保持相关且符合窗口大小。
...
没有。Previously 是实验性的 v0.4.0。README 带有"实验性"状态徽章，并明确说明"尚未准备好用于个人或生产用途"。`APP_VERSION` 常量是 `0.4.0`。
...
项目自身的约定将代码和数据放在同一个仓库中——其中 `src/` 为 agent 只读，而数据目录（`memory/`、`tasks/`、`sessions/`）为 agent 读写。但配置给了你选择权。独立的记忆仓库也是可行的。

## 运行成本是多少？

两个成本中心：

- **LLM API 使用费**——Previously 使用你自己的 API 密钥（DeepSeek）运行。两层架构（Flash 用于快速召回扫描，Pro 用于深度推理）是出于成本考虑：廉价模型处理常见情况，昂贵的推理保留给需要的时候。

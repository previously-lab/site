...
| `ANTHROPIC_API_KEY` | 否 | 仅路线图 | — | 出现在 `README.md` 中，`@ai-sdk/anthropic` 依赖也已安装，但**已发布的代码中没有任何地方读取 `process.env.ANTHROPIC_API_KEY`** 或实例化 Anthropic 提供者。多提供者支持在模型注册表中已有类型定义（`provider: "deepseek" \| "anthropic" \| "openai"`），但 `DEFAULT_MODELS` 只包含 DeepSeek 条目。在 v0.3.0 中设置此变量没有任何运行时效果。 |
...
> `DEMO_REF` 存在于代码中，但**在 `.env.example` 中没有记录**——这是 v0.3.0 中的一个文档缺口。

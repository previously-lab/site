...
内核以 [`previously-kernel`](https://www.npmjs.com/package/previously-kernel) 发布在 npm——它是 agent 仓库（previously-lab/agent）的预构建 standalone 产物。client 把它钉死：同一个版本字符串同时出现在 `dependencies` 和 `previously.kernelVersion` 里——没有范围号，没有"兼容 ^0.9"，patch 位也不例外。

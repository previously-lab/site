---
title: 架构
description: Previously 是部署在 Vercel 上的一个 HTTP 端点，它读取 GitHub 文件，调用两个 LLM，并将一个响应流式传回——没有数据库，没有定时任务，没有持久化服务器。
order: 11
---
...
没有数据库，没有常驻 agent，没有持久连接。Previously 完全运行在 Vercel 的请求生命周期内。你发送一条消息，路由触发管道，响应流式传回。当响应结束时，没有后台进程，没有心跳，没有循环。
</Callout>

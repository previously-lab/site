---
title: Introduction
description: Previously is an AI agent that remembers by when — conversations become slices on one timeline, recall answers with verbatim evidence, and your memory is a plain folder you own.
---

# Introduction

Previously is an AI agent that lives on your own machine. There is exactly one fundamental difference between it and every other AI assistant, and that difference changes everything: **it remembers by *when*, not by chat thread.**
...
## It runs on your machine, and the memory is yours

The whole product is one npm package running on your own hardware. Your memory is a plain folder — pure Markdown with a local git ledger — that belongs to you from day one: back it up, move it, open it with any tool.
...
- [Getting Started](/docs/getting-started) — two commands to run it on your machine

...
Previously is not another assistant you chat with in disposable threads. It is a persistent cloud agent organized around a single idea: **one continuous relationship on one timeline.** You show up, you talk, you leave. When you come back — hours, days, or weeks later — the first thing you see is a vertical timeline, not a list of chat threads. The agent determines relevance by scanning *when* something happened, then retrieving *what* was said.

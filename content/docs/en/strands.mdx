...
As of v0.3.1 the strand index is **write-only**. It is built and maintained, but no recall code programmatically reads `strands.json` to drive Flash's recall scan. The only path that surfaces strands to the model is a prompt hint added to the Pro context:

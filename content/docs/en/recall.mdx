...
Every step recall takes — reading a timeline window, tracing a strand, opening a slice, landing its report — renders live on a tool card in the chat: recall runs in the open, not behind a spinner. The three rendering phases and the five card states are covered in [The Chat Interface](/docs/chat-ui).
...
- [Evolution](/docs/evolution) — the loop that improves recall's playbook when the fitness score says so

...
**`DEEPSEEK_API_KEY` is required.** The web-search infrastructure call always goes through DeepSeek, regardless of which model you chat with. Without that key, `webSearch` fails with an explicit error instead of silently degrading into a guess.
</Callout>

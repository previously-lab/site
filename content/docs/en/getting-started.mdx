...
Previously does not have an in-app onboarding wizard. Instead, it auto-detects your environment: if you have a GitHub token configured, you are using your own timeline; if not, you land in demo mode — no setup required.

The live site at [previously.ldwid.com](https://previously.ldwid.com) **is** the demo. Visit without a GitHub token and you are automatically exploring 20 personas from the [WorldMemArena](https://github.com/WorldMemArena/worldmemarena) benchmark (CC BY-NC 4.0), each with 30+ slices spanning years of conversations. No separate demo subdomain, no env vars, no configuration.

A small **"Demo" badge** (or "体验" in Chinese) appears in the header to remind you that you are in read-only mode. Click it for a popover explaining demo mode and linking to the setup guide. Messages you send are not persisted — refresh the page and the slate resets.

### Exploring the Personas

The hero at the top of the page shows "Previously on {persona name}". Click the persona name to open a dialog with a grid of 20 persona cards. Each card shows the persona's name and a short blurb describing their life story. Click any card to switch personas — the page does a hard refresh with `?persona=personal_07` in the URL, and the timeline reloads with that persona's conversations.

Every persona runs the full two-tier recall system (Flash + Pro), so you can browse their timeline, scroll through past slices, chat freely, and watch recall work — all without setting up your own deployment.

### Streaming Page Structure

The page streams in three layers so nothing flashes or jumps:

- **"Previously on" title** — static, never re-renders
- **Persona name** — wrapped in a Suspense boundary, loads after the title
- **Timeline** — in its own Suspense boundary, loads independently

### Data Source

The benchmark data is not bundled in the agent repository. It is fetched remotely from a configurable public GitHub repository (see `BENCHMARK_BASE_URL`). This keeps the agent repo lean and allows the dataset to be updated independently.

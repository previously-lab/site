---
title: Memory Model
description: How Previously structures memory across episodic slices, semantic strands, and knowledge nodes — all stored as plain Markdown files in a git repository.
order: 3
---
...
A calendar day is a directory that may hold multiple slice files. A slice opens when you start talking, stays active while you stay engaged, and closes automatically after 30 minutes of silence. Read top-to-bottom across months and years, slices are your autobiography — what happened, when it happened, in chronological order.
...
When Previously constructs a prompt, the context assembler packs a 5-layer payload under a default budget of 8,000 tokens:

| Layer | Content | Budget Rule |
|-------|---------|-------------|
| 0 | System prompt (constitution + directives) | Full, always included |
| 1 | Core memory nodes (full content) | Capped at budget x 0.7 (reserves 30% for later layers) |

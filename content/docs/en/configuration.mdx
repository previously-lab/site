...
| `ANTHROPIC_API_KEY` | No | Roadmap only | — | Appears in `README.md` and the `@ai-sdk/anthropic` dependency is installed, but **no shipped code reads `process.env.ANTHROPIC_API_KEY`** or instantiates an Anthropic provider. Multi-provider support is typed in the model registry (`provider: "deepseek" | "anthropic" | "openai"`) but `DEFAULT_MODELS` ships only DeepSeek entries. Setting this variable has zero runtime effect in v0.3.1. |
...
> `DEMO_REF` is present in the code but **not documented in `.env.example`** — a documentation gap in v0.3.1.
...
| `LOG_LEVEL` | Not implemented. Logging is thin in v0.3.1. |

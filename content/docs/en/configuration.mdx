...
Four environment variables are required for the GitHub backend: DEEPSEEK_API_KEY, GITHUB_TOKEN, GITHUB_REPO_OWNER, and GITHUB_REPO_NAME. Without GITHUB_TOKEN, the app reads and writes the local filesystem instead.
</Callout>
...
| `GITHUB_REPO_NAME` | When using GitHub backend | Yes | `local` | The repository name for your private repository. Same consumption points as `GITHUB_REPO_OWNER`. |
...
| `ANTHROPIC_API_KEY` | No | Roadmap only | — | Appears in `README.md` and the `@ai-sdk/anthropic` dependency is installed, but **no shipped code reads `process.env.ANTHROPIC_API_KEY`** or instantiates an Anthropic provider. Multi-provider support is typed in the model registry (`provider: "deepseek" | "anthropic" | "openai"`) but `DEFAULT_MODELS` ships only DeepSeek entries. Setting this variable has zero runtime effect in v0.3.0. |
...
> `DEMO_REF` is present in the code but **not documented in `.env.example`** — a documentation gap in v0.3.0.
...
| `LOG_LEVEL` | Not implemented. Logging is thin in v0.3.0. |

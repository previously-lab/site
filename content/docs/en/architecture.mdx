...
Every chat turn runs inside a durable Vercel Workflow run — resumable after a dropped connection, with each LLM call and tool call as an individually retried step.
</Callout>

Previously is powered by Vercel Workflow for durable execution — each chat turn reads GitHub files, calls two LLMs, and streams one response back through a resumable run. No database, no cron, no persistent server.
...
Every chat turn runs inside a durable Vercel Workflow run (`src/app/api/chat/turn-workflow.ts`). This means:

- **Resumable streaming** — the response streams through `run.readable`. If the user drops their connection, the workflow continues running on Vercel's infrastructure and the client can reconnect to the same turn.
...
- **WorkflowAgent** — the agent loop uses AI SDK 7's `WorkflowAgent` (`@ai-sdk/workflow`). Chat turns and background loops share one agent layer (`src/app/api/agent/` — factories, tool definitions, and standalone tool executors).

Background loops (`src/app/api/loops/`) use the same Workflow infrastructure. The agent can spawn durable background tasks that persist their progress to the repo. GitHub files remain the single source of truth — Workflow is the execution container, never a store.
...
The page uses a streaming structure built on React Server Components with Suspense boundaries. Key sections of the page — such as the timeline panel and the chat message area — are wrapped in `<Suspense>` with loading fallbacks, enabling the page shell to render immediately while data-dependent sections stream in as they resolve. This replaces the older pattern of fetching all data synchronously in the server component before rendering.

The Next.js App Router enforces a strict server-component/client-component boundary. `page.tsx` is a Server Component that owns the streaming layout. `ChatPage` is a thin client shell that owns only the AI SDK `useChat` hook and the flex layout. The hero section and timeline panel are passed as React children from `page.tsx`, keeping the React Server Component boundary narrow.
...
When `GITHUB_TOKEN` is set, these tools use Octokit against the user's GitHub repo. The data-source layer (`resolveDataSource()`) auto-detects whether the app is running in GitHub-connected mode, local development mode, or demo mode (remote fetch from benchmark repo).
...
| GitHub Tools | `src/lib/tools/` | Octokit-backed readFile/writeFile/listFiles with local-fs fallback for local dev; demo mode uses remote fetch from benchmark repo |
| Data Source | `src/lib/data-source/` | Auto-detects GitHub (user data) vs remote demo fetch (20 benchmark personas) via `resolveDataSource()` |
| Version & Sync | `src/lib/version/` | Version check against upstream releases, in-app code sync via Git Data API |
| Episodic Subsystem | `src/lib/episodic/` | Time-slice management, unified Flash, strand indexing |

### Skills System

Skills are file-driven agent capabilities defined in `.claude/skills/` directories:

- **Discovery** (`src/lib/skills/discovery.ts`) — scans directories for `SKILL.md` files, parses YAML frontmatter
- **Loading** (`src/lib/skills/loader.ts`) — extracts the skill body, substitutes `$ARGUMENTS`
- **Registry** (`src/lib/skills/registry.ts`) — merges programmatic and discovered skills into one lookup table

A built-in `/create-memory` skill lets the agent create structured memory nodes in `memory/nodes/`. Skills are a lightweight extension point — no plugin system, no runtime compilation, just Markdown files with YAML metadata.

### Version & Sync

Previously includes a built-in version check and code sync mechanism:

- **Version check** (`src/lib/version/actions.ts`) — a server action that fetches the latest release tag from the upstream template repository's GitHub Releases API and compares it with the running `APP_VERSION` using semver. Cached for 1 hour.
- **Sync from upstream** (`src/lib/version/sync.ts`) — a server action that uses the Git Data API to construct a proper merge commit with two parents (the user's HEAD and the upstream HEAD). Only code directories (`src/`, `content/`, `public/`, etc.) are synced — user data in `memory/`, `tasks/`, and `sessions/` is never touched.

The sync mechanism is triggered from Settings → Version & Updates. It creates a merge commit preserving git history, so subsequent syncs can build on the previous merge point.
...
### Data-Source Auto-Detection (v0.4.0)

Previously v0.4.0 removes the `DEMO_MODE` environment variable and its associated path redirection. Instead, `resolveDataSource()` in `src/lib/data-source/resolve.ts` auto-detects the active data source at runtime. When `GITHUB_TOKEN` is set, the system uses GitHub. Without a token, it checks whether demo mode should activate: demo data (20 WorldMemArena benchmark personas, switched via `?persona=` URL parameter) is fetched from a public benchmark repo defined by `BENCHMARK_BASE_URL`, while local development uses the filesystem directly. A demo badge in the AppHeader indicates demo mode is active.

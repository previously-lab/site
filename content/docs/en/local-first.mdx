...
Previously has one kernel and two ways to run it. In the local form, the kernel is packaged as the npm package [`previously-kernel`](https://www.npmjs.com/package/previously-kernel), distributed pinned to an exact version by the client — what runs on your machine is the same code a cloud deployment runs.
